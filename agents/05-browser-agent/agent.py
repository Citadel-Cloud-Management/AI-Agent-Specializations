"""BrowserAgent - Performs tasks on websites automatically."""
...
CFG = AgentConfig("browser-agent", "Performs tasks on websites automatically")
...
class BrowserAgent(BaseAgent):
    ...
    async def execute(self, d: dict) -> AgentResult:
        ...
        self._log(f"Executing browser-agent")
        resp = await self._llm.complete(LLMRequest(
            str(d["goal"]), system="You are a browser automation agent. Plan steps to achieve the goal. Return JSON.", temperature=0.2,
        ))
        try:
