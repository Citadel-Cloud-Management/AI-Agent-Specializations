"""SecurityGovernanceAgent - Automated threat triage, compliance validation, IAM intelligence."""
...
CFG = AgentConfig("security-governance-agent", "Automated threat triage, compliance validation, IAM intelligence")
...
class SecurityGovernanceAgent(BaseAgent):
    ...
    async def execute(self, d: dict) -> AgentResult:
        ...
        self._log(f"Executing security-governance-agent")
        resp = await self._llm.complete(LLMRequest(
            str(d["action"]), system="You are a SOC analyst. Triage threats, validate compliance. Return JSON.", temperature=0.2,
        ))
        try:
