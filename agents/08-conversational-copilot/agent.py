"""ConversationalCopilot - Enterprise knowledge assistants and workflow copilots."""
...
CFG = AgentConfig("conversational-copilot", "Enterprise knowledge assistants and workflow copilots")

class ConversationalCopilot(BaseAgent):
    ...
    async def execute(self, d: dict) -> AgentResult:
        ...
        self._log(f"Executing conversational-copilot")
        resp = await self._llm.complete(LLMRequest(
            str(d["message"]), system="You are an enterprise copilot. Answer from knowledge base accurately.", temperature=0.2,
        ))
        try:
