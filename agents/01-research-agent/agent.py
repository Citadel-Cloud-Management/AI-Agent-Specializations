"""ResearchAgent - Finds, summarizes, and compares research papers, blogs, docs."""
...
CFG = AgentConfig("research-agent", "Finds, summarizes, and compares research papers, blogs, docs")
...
class ResearchAgent(BaseAgent):
    ...
    async def execute(self, d: dict) -> AgentResult:
        ...
        self._log(f"Executing research-agent")
        resp = await self._llm.complete(LLMRequest(
            str(d["query"]), system="You are a research agent. Find, summarize and compare sources. Return JSON.", temperature=0.2,
        ))
        try:
