"""InfrastructureAgent - Autonomous cloud ops, auto-healing clusters, FinOps optimization."""
...
CFG = AgentConfig("infrastructure-agent", "Autonomous cloud ops, auto-healing clusters, FinOps optimization")
...
class InfrastructureAgent(BaseAgent):
    ...
    async def execute(self, d: dict) -> AgentResult:
        ...
        self._log(f"Executing infrastructure-agent")
        resp = await self._llm.complete(LLMRequest(
            str(d["action"]), system="You are an SRE agent. Auto-heal clusters, optimize costs. Return JSON.", temperature=0.2,
        ))
        try:
