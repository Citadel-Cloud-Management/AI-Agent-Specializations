"""DataAnalyticsAgent - Intelligent analytics and RAG with NLQ across BI platforms."""
...
CFG = AgentConfig("data-analytics-agent", "Intelligent analytics and RAG with NLQ across BI platforms")
...
class DataAnalyticsAgent(BaseAgent):
    ...
    async def execute(self, d: dict) -> AgentResult:
        ...
        self._log(f"Executing data-analytics-agent")
        resp = await self._llm.complete(LLMRequest(
            str(d["query"]), system="You are a data analytics agent. Convert NL to SQL, detect anomalies. Return JSON.", temperature=0.2,
        ))
        try:
